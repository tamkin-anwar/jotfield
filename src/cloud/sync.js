...
async function encryptNotebook(notebook, key, salt) {
...
async function decryptNotebook(row, key) {
...
export function createEncryptedSync({ clientPromise, getNotebook, mergeNotebook, applyNotebook, setStatus, openDatabase }) {
  ...
  async function start(nextSession) {
    session = nextSession;
    if (!session) { key = null; workspaceId = null; setStatus('off'); return; }
  ...
  return { start, unlock, schedule, push, lock, isUnlocked: () => Boolean(key) };
}
